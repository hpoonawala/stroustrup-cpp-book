...
int main(){
	...
	int nmax;
	cin >> nmax;
	int nums[nmax]; // Initialize as 0s. We'll mark non-primes with a -1
	for(int i=0;i<nmax;i++) nums[i]=0;
	...
	while(cursor<nmax){
		...
		if(nums[cursor]==0){ // only if cursor+1 is unmarked should we mark its multiples
			for(int i=cursor*(cursor+2);i<nmax;i=i+cursor+1){ // Mark all multiples of number cursor+1
				/* cout << i << ","; */
				nums[i]=-1;
			}
			/* cout<<"\n"; */
